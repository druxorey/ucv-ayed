...
/**
 * @brief Estructura de Lista Circular (Ring).
 *
 ...
 * y encapsula un iterador
 * ('it') que avanza indefinidamente de manera circular.
 *
 * @tparam T Tipo de dato de los elementos en el anillo.
 */
template <typename T> class DrxRing : public DrxList<T> {
  public:
	/**
	 * @brief Constructor: Inicializa el anillo y coloca el cursor inicial en el
	 * fin (nullptr).
	 */
	DrxRing() : DrxList<T>() { it = this->begin(); }

	/**
	 * @brief Añade un nuevo elemento al anillo.
	 * Si es el primer elemento de la lista circular, posiciona el cursor sobre
	 * él.
	 */
	...
	/**
	 * @brief Remueve el elemento en la posición actual del cursor.
	...
	/**
	 * @brief Avanza el cursor y retorna el elemento actual.
